...
get_two_group <- function(dt_exp_annot, dt_pairs, result_dir){
  ...
  degs_performance_test <- unique(pairs_sample[['compare_group']]) %>% lapply(., function(x){
    ...
    ref_pos = ref_degs[.(x, 'deg'), on = .(compare_group, checkout)][['gene']]
    ref_neg = ref_degs[.(x, 'non-deg'), on = .(compare_group, checkout)][['gene']]
  ...
  ref_degs_per_m <- ref_degs_per[unique(ref_meta[, c('Batch', 'symbol.libPrep')]), on = "Batch"]
  ...
  rel_exp_per_test <- unique(dt_DEG[['compare_group']]) %>% lapply(., function(x){
    int_gene = intersect(dt_DEG[x, on = .(compare_group)][['gene']], ref_range[x, on = .(compare_group)][['gene']])
    dt_DEG_int = dt_DEG[.(x, int_gene), on = .(compare_group, gene)]
    ref_range_int = ref_range[.(x, int_gene), on = .(compare_group, gene)]
  ...
  setnames(ref_rel_exp_per, 'batch.x', 'Batch')
  ref_degs_per_m <- ref_rel_exp_per[unique(ref_meta[, c('Batch', 'symbol.libPrep')]), on = "Batch"]
  setnames(ref_degs_per_m, 'symbol.libPrep', 'Group')
