get_one_group <- function(dt_exp_annot, dt_pairs, result_dir){
  ...
  ref_detected_gene_per$Group <- 'reference'
  ...
  detected_gene_performance_test <- unique(dt_exp_annot[['group']]) %>% lapply(., function(x){
    ...
    ref_pos = ref_data[.(sample_type, 'Pos_tier1'), on = .(Sample, Type)][['Gene']]
    ref_neg = ref_data[.(sample_type, 'Neg_tier1'), on = .(Sample, Type)][['Gene']]
  ...
  #
  detected_gene_performance_compared <- rbind(ref_detected_gene_per, detected_gene_performance_test)
  ...
  SD_ref_compare <- rbind(SD_ref[, !"SNR"], dt_test_sd)
  ...
  studydesign_performance_summary <- data.table('Term' = c('Detection Jacard Index (JI)', 'Coefficient of variation (CV)', 
  ...
  detected_gene_performance_mean <- lapply(highqual_batch, function(x){
